...
namespace Asc
{
    class TestCsvFileAnalyzer
    {
        ...
        public void UC07_ReadPM2_5_Csv_20180101()
        {
            ...
            List<string> List_deviceId = CreateDeviceIdList();
            CreateQmtAndPst(List_deviceId,
            ...
            Dal.Def = Def;
            Dal.Dic_Qmt = Dic_Qmt;
            Dal.DeviceIdList = List_deviceId;
        ...
        public void UC08_AggregateFromQmtToPst()
        {
            ...
            foreach (KeyValuePair<string, QuickMixTank> entry in Dic_Qmt)
            {
                ...
                // Debug
                if (isToPrint(entry.Key))
                {
                    PrintQmt(entry.Value, FilePath);
                    PrintPst(Pst, FilePath);
                }
                // Reset
                entry.Value.Reset();
            }
            ...
            // DataAvg
            Assert.AreEqual(24.46821841, Dic_Pst["MM01"].DataAvg, 0.1);
            Assert.AreEqual(24.46821841, Dic_Pst["ww01"].DataAvg, 0.1);
            Assert.AreEqual(24.46821841, Dic_Pst["dd01"].DataAvg, 0.1);
            Assert.AreEqual(20.13677468, Dic_Pst["HH01"].DataAvg, 0.1);
            // DataAmount
            Assert.AreEqual(5308, Dic_Pst["HH01"].DataAmount);
            Assert.AreEqual(128801, Dic_Pst["MM01"].DataAmount);
            // 初值為零
            Assert.AreEqual(0, Dic_Pst["dd02"].DataAvg, 0.1);
            ...
            // 時間模式檢驗 for all stations (not only Taichung)
            Assert.AreEqual(25.29205887, Dic_Pst["MM01"].DataAvg, 0.1);     // changed
            Assert.AreEqual(278235, Dic_Pst["MM01"].DataAmount);            // changed
            Assert.AreEqual(24.46821841, Dic_Pst["ww01"].DataAvg, 0.1);     // unchanged
            Assert.AreEqual(128801, Dic_Pst["ww01"].DataAmount);            // unchanged
            Assert.AreEqual(24.46821841, Dic_Pst["dd01"].DataAvg, 0.1);     // unchanged
            Assert.AreEqual(128801, Dic_Pst["dd01"].DataAmount);            // unchanged
            Assert.AreEqual(25.91038130, Dic_Pst["HH01"].DataAvg, 0.1);     // changed
            Assert.AreEqual(10779, Dic_Pst["HH01"].DataAmount);             // changed

            // 初值不為零
            // Amount = 149434, Avg = 26.00214811
            Assert.AreEqual(26.00214811, Dic_Pst["dd02"].DataAvg, 0.1);
            Assert.AreEqual(149434, Dic_Pst["dd02"].DataAmount);
        ...
        public void UC10_Aggregate2018_OneYear()
        {
            ...
            List<string> TankIdList = CreateTankIdList(DeviceIdList);
            ...
            {
                // [FileName]_[RowId], e.g., [Y2018S04]_[DeviceId], [Day]_[dd01], [Hour]_[HH00], [Month]_[MM01], [Week]_[ww00], 
        ...
        private List<string> CreateTankIdList_TemporalMode()
        {
            ...
            for (int i = 1; i <= 12; i++)
            {
                string key = "Month_" + "MM" + i.ToString("00");
                TankIdList.Add(key);
            }
            for (int i = 0; i <= 6; i++)
            {
                string key = "Week_" + "ww" + i.ToString("00");
                TankIdList.Add(key);
            }
            for (int i = 1; i <= 31; i++)
            {
                string key = "Day_" + "dd" + i.ToString("00");
                TankIdList.Add(key);
            }
            for (int i = 0; i <= 23; i++)
            {
                string key = "Hour_" + "HH" + i.ToString("00");
                TankIdList.Add(key);
            }
            return TankIdList;
        }
        ...
        private List<string> CreateTankIdList(List<string> DeviceIdList)
        {
            List<string> TankIdList = new List<string>();
            TankIdList.AddRange(CreateTankIdList_TemporalMode());
            ...
            //for (int i = 1; i <= 12; i++)
            //{
            //    string key = "MM" + i.ToString("00");
            //    TankIdList.Add(key);
            //}
            //for (int i = 0; i <= 6; i++)
            //{
            //    string key = "ww" + i.ToString("00");
            //    TankIdList.Add(key);
            //}
            //for (int i = 1; i <= 31; i++)
            //{
            //    string key = "dd" + i.ToString("00");
            //    TankIdList.Add(key);
            //}
            //for (int i = 0; i <= 23; i++)
            //{
            //    string key = "HH" + i.ToString("00");
            //    TankIdList.Add(key);
            //}
            //#endregion
            ...
            //    for (int i = 2018; i <= 2020; i++)
            //    {
            //        string key = "Y" + i.ToString("0000") + "_" + DeviceId;
            //        TankIdList.Add(key);
            //    }
            //    // 季均值
            //    for (int i = 2018; i <= 2020; i++)
            ...
            //        for (int j = 1; j <= 4; j++)
            //        {
            //            string key = "Y" + i.ToString("0000") + "S" + j.ToString("00") + "_" + DeviceId;
            //            TankIdList.Add(key);
            //        }
            //    }
            //}
            ...
            out Dictionary<string, PrimarySedimentationTank> Dic_Pst)
        {
            ...
            //{
            //    string key = "MM" + i.ToString("00");
            //    Dic_Qmt.Add(key, new QuickMixTank(key));
            //    Dic_Pst.Add(key, new PrimarySedimentationTank(key));
            //}
            //for (int i = 0; i <= 6; i++)
            //{
            //    string key = "ww" + i.ToString("00");
