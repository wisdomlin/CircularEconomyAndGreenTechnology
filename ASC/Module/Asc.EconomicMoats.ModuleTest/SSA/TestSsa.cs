...
namespace EconomicMoat.ModuleTest
{
    class TestSsa
    {
        ...
        [Test]
        public void UC02_FpiDataDecomp()
        {
            //
            // Here we demonstrate SSA trend/noise separation for some toy problem:
            // small monotonically growing series X are analyzed with 3-tick window
            ...
            double[] trend;
            double[] noise;
            alglib.ssaanalyzesequence(s, Fpi, out trend, out noise);
            System.Console.WriteLine("{0}", alglib.ap.format(trend, 2));
            System.Console.WriteLine("{0}", alglib.ap.format(noise, 2));

            string FilePath = @"C:\Workspace\Branches\CircularEconomyAndGreenTechnology\EconomicMoat\EconomicMoat\EconomicMoats.ModuleTest\SSA\Result\" +
                "result" + DateTime.Now.ToString("_yyyyMMdd-HHmm-ss_") + ".csv";
        ...
        [Test]
        public void UC03_FpiDataFromCsv()
        {
            ...
            using (StreamReader sr = new StreamReader(bs))
            {
                ...
                while ((Line = sr.ReadLine()) != null)
                {
                    ...
                    Double.TryParse(LineSplits[1], out Double ParseResult);
                    IndexList.Add(ParseResult);
                }
            }
            string[] DateArray = DateList.ToArray();
            double[] IndexArray = IndexList.ToArray();
        }
    }
}
